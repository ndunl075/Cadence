...
function renderSvg(report, options = {}) {
  ...
  const rects = report.daily.map((day, index) => {
    ...
    const fill = palette[level(day.tokens, max)];
    return `<rect x="${x}" y="${y}" width="${cell}" height="${cell}" rx="2" fill="${fill}"><title>${escape(day.date)} · ${day.tokens.toLocaleString()} tokens</title></rect>`;
  }).join('');
  ...
  <text x="28" y="55" class="muted" font-size="11">${report.totals.tokens.toLocaleString()} tokens · ${report.activeDays} active days · ${report.longestStreak} day streak</text>
  ...
  <text x="28" y="${height - 18}" class="muted" font-size="9">CADENCE · ${report.year}</text>
</svg>`;
}
